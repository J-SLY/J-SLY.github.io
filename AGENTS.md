...
- **JS load order** (index.html:149–161): `marked.min.js` → `pinyin-pro` → `display-mode.js` → `article-content.js` → `article-modal.js` → `articles.js` → `search.js` → `dark-mode.js` → `settings.js` → `navigation.js` → `main.js`
...
-- **JS for standalone article page** (standalone route `/article/{id}` served via `404.html` router): `display-mode.js` → `article-content.js` → `dark-mode.js` → `search.js` (rendering happens in the 404 router script)
